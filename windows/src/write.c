...
int write_archive_file(PAR3_CTX *par3_ctx, char command_recovery_file_scheme)
{
	...
	if (file_count > 0){	// Number of input block files is same as recovery block files.
		if (command_recovery_file_scheme == 'u'){	// Uniform
		...
		} else if (command_recovery_file_scheme == 'l'){	// Limit size
