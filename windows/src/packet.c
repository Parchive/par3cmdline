...
int make_start_packet(PAR3_CTX *par3_ctx)
{
	...
	par3_ctx->start_packet_size = (size_t)(tmp_p - par3_ctx->start_packet);	// packet size
	//printf("Start Packet body size = %zu\n", par3_ctx->start_packet_size - 48);

	// Max size of Start Packet is 48 + 8 + 16 + 8 + 1 + 2 = 83 bytes.
	// This supports 8-bit or 16-bit Galois Field.
	if (par3_ctx->start_packet_size > 83){
		return RET_LOGIC_ERROR;
	}
	...
	if (par3_ctx->noise_level >= 2){
		printf("Input Set ID = %02X %02X %02X %02X %02X %02X %02X %02X\n",
...
int make_file_packet(PAR3_CTX *par3_ctx)
{
	...
	absolute_num = 0;
	if (par3_ctx->attribute & 1){	// Enable absolute path
		absolute_num = 1;
		tmp_p = par3_ctx->base_path;
		while (tmp_p[0] != 0){
			if (tmp_p[0] == '/')
	...
	if (num > 0){
		...
		// Reduce memory usage to used size.
...
int make_ext_data_packet(PAR3_CTX *par3_ctx)
{
	...
	if (par3_ctx->noise_level >= 2){
		printf("Number of External Data Packets = %zu (%I64d blocks)\n", write_packet_count, find_block_count);
	}
...
int duplicate_common_packet(PAR3_CTX *par3_ctx)
{
	...
	if (par3_ctx->matrix_packet_size > 0){
		...
		packet_count++;	// Number of Matrix Packets is 1 at this time.
	}
