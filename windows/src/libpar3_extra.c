	...
	while (offset + 48 < total_size){
		...
		if (memcmp(packet_type, "PAR CAU\0", 8) == 0){	// Cauchy Matrix Packet
			...
			if (par3_ctx->noise_level >= 1){
				...
				printf("Index of first input block         = %I64u\n", first_num);
				printf("Index of last input block plus 1   = %I64u\n", last_num);
				printf("hint for number of recovery blocks = %I64u\n", hint_num);
				printf("\n");
			}
			...
			if (par3_ctx->noise_level >= 1){
				...
				printf("Index of first input block       = %I64u\n", first_num);
				printf("Index of last input block plus 1 = %I64u\n", last_num);
				printf("Max number of recovery blocks    = %I64u\n", max_num);
